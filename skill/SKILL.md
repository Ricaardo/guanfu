...
#### `ahr999_compressed`（估值主信号）
- **定义**：固定公式 AHR999 的压缩版，`compressed = raw^0.75`。它保留原始 AHR 的分档方向，但压低 `price^2` 带来的凸性偏差。
- **推荐用途**：优先用于估值投票、底部接近度和顶部风险判断；`ahr999` 自适应版仅作辅助确认。
- **等价阈值**：
  - < 0.550 → 极端抄底（对应 raw < 0.45）
  - 0.550-0.846 → 低估 / 经典定投区（对应 raw 0.45-0.8）
  - 0.846-1.147 → 合理估值（对应 raw 0.8-1.2）
  - 1.147-1.682 → 高估（对应 raw 1.2-2.0）
  - > 1.682 → 泡沫（对应 raw > 2.0）
  - > 3.344 → 极端泡沫（对应 raw > 5.0）
- **经验依据**：压缩版用于修正 2024 后波动率/机构结构变化下原版 AHR 顶部桶误判问题。

#### `ahr999`（自适应辅助信号）
- **定义**：(price / 200d-DCA-cost) × (price / fitted-fair-value)。本实现使用调和均值 DCA、动态加权回归、动态分位数和 Huber IRLS。
- **用途**：观察自适应估值线和历史分位变化，不作为域级估值投票的首要阈值。
- **联动**：与 Mayer 高度相关；与 MVRV 互补（MVRV 用链上成本，AHR 用价格 DCA）。

#### `ahr999_divergence`
- **定义**：检测固定公式 AHR999 与自适应 AHR999 百分位方向是否冲突。
- **用途**：当压缩版与自适应版给出相反区间时，降低估值结论置信度，优先等待 MVRV/NUPL/资金流确认。
...
#### `tlt_price` / `tlt_60d_trend_pct` (Futu)
- **定义**：iShares 20+ Year Treasury Bond ETF (TLT)。**长端美债价格代理，与 30Y 收益率反向**。来源 Futu US.TLT。
- **机制**：长端利率主要驱动 BTC 估值分母（无现金流资产的折现率）。TLT 下跌 = 长端利率上行 = BTC 估值压力上升。
- **历史区间**（2010-2025）：COVID 高点 ~170（极低利率），2022-23 紧缩低点 ~80（30Y 收益率冲到 ~5%）。
- **`tlt_price` 阈值**：
  - `< 85`：长端利率高位，BTC 估值压力大
  - `85-95`：偏低
  - `95-110`：中性
  - `110-130`：偏高
  - `> 130`：长端深度低利率，BTC 顺风
- **`tlt_60d_trend_pct` 阈值**（更适合判断方向）：
  - `< -10%`：长端利率冲击（如 2022-Q3 / 2023-Q3 收益率快速上行）
  - `-10% ~ -5%`：利率走高
  - `-5% ~ +5%`：横盘
  - `+5% ~ +10%`：利率回落
  - `> +10%`：衰退避险或政策转鸽（如 2024-Q3 降息预期）
- **联动**：
  - TLT 趋势上行 + `real_yield_10y_pct` 回落 + `dxy_60d_trend_pct < 0` = 三重宽松信号
  - TLT 暴跌 + `hy_spread_bps` 扩大 = 长端利率上行 + 信用压力（2022-Q3 典型）
- **失效**：TLT 是名义债券价格，不直接反映通胀预期。需结合 `real_yield_10y_pct`（实际利率）一起读 — 名义利率上行可能来自实际利率上升或通胀预期上升，含义不同。
...
| **-3 ~ -4** | **高防守倾向** | 多数域看跌。优先讨论风险暴露是否过高，但不替用户指定仓位比例。 |
| **≤ -5** | **分配/避险风险** | 5+ 域一致看跌。强调风险控制、反证和退出条件，不输出“清仓”式指令。 |
...
6. **不替代 trade-execution** — 仓位规模、止损位置等执行细节交给执行层处理
7. **不输出无上下文交易指令** — 用户没有给出持仓、期限、风险预算时，不说“立刻买/卖/清仓”
...
- ❌ **看到 phase=accumulation 就 ALL IN** — phase 是粗分类，必须配合具体指标 + 风险预算（执行层另行处理）
