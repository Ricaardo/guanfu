...
## kNN 预测引擎 + 可靠性标注(post-refresh CBOE baseline,2026-05-11)
...
`guanfu --forecast` 使用每个资产自己的 feature bundle 做 historical analogue / kNN：BTC 用价格/周期/估值特征；QQQ/SPY 用技术、CAPE、FRED 宏观、VIXY 和 CBOE put/call；Gold 用技术、实际利率、通胀预期、DXY、COT 和 VIXY；任意美股用技术 + 通用宏观。引擎会把当前状态与历史日期做加权距离匹配，再统计相似样本之后 30/90/180 天等 horizon 的前向收益分布。
