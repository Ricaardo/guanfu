...
| 目录 | 用途 | 调用方式 |
|---|---|---|
| `guanfu/` | 主 CLI + MCP 可调的多资产盘面 | `guanfu [btc\|qqq\|spy\|gold\|stock TICKER] [--verdict\|--forecast\|--full]`;子命令 `refresh` / `market` / `dca` / `allocate` / `intent` / `watch` / `digest` / `calibrate` / `backtest` / `status --frank` |
