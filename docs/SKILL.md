...
name: btc-guanfu
description: |
  观复 / 观察万物之周期回归 — BTC 投资盘面 + 解读知识库。输出 8 个域（cycle / valuation / network / positioning / macro / flow / technical / cross_asset）的纯指标盘，每个指标含原始值、历史分位、解读标签、数据源。**不输出评分 / action / state** — 决策由 Claude 综合盘面 + 本 skill 知识库 + 用户上下文完成。
...
#### `uup_price` / `btc_uup_corr_30d` (v3.1)
- **uup_price**：做多美元 ETF (DXY 代理)。上涨 = DXY 走强 = BTC 逆风。来源 Futu US.UUP。
- **btc_uup_corr_30d**：BTC vs UUP 30 日相关。负值 = 经典逆向关系（美元强 → BTC 弱）。

#### `vixy_price` (v3.1)
- **定义**：VIX 短期期货 ETF（恐慌指数）。来源 Futu US.VIXY。
- **阈值**：< 15 极度平静（自满），15-20 低波，20-25 正常，25-30 偏高，> 30 极高恐慌 → 风险资产承压。
- **联动**：VIXY > 30 + F&G < 20 + funding < 0 = 全市场恐慌底。

#### `gld_etf_price` (v3.1)
- **定义**：GLD 实物黄金 ETF。来源 Futu US.GLD。与 Binance PAXG 交叉验证（两者应高度同步）。
