...
name: btc-guanfu
description: |
  观复 / 观察万物之周期回归 — BTC 投资盘面 + 解读知识库。输出 6 个域（cycle / valuation / network / positioning / macro / flow）的纯指标盘，每个指标含原始值、历史分位、解读标签、数据源。**不输出评分 / action / state** — 决策由 Claude 综合盘面 + 本 skill 知识库 + 用户上下文完成。
  当用户问「BTC 该不该买/卖」、「比特币现在估值如何」、「加密底/顶在哪」、「定投区吗」、「AHR999/MVRV/哈希率/ETF 流入多少」、「BTC 周期位置」、「观复」时触发。
  NOT for: 仅查 BTC 实时价格 → cmc-mcp；altcoin/memecoin → cmc-mcp / okx-dex；技术分析图表 → technical-analysis；链上钱包 → okx-wallet。
license: MIT
user-invocable: true
required_tools: []
---
...
- **万物并作** ↔ 6 个 domain × N 个指标（周期 / 估值 / 链上 / 杠杆 / 宏观 / 资金流）同时呈现
- **观复** ↔ 在历史分位中观察其往复 —— halving 周期回环、估值高低反覆、流入流出来去
- **致虚守静** ↔ 不输出评分 / action / state，不替代主人的"静"。决策权归于人

具体分工：

- 二进制（`/Users/x/news/bin/btc-guanfu`）= 数据采集 + 指标计算 + 历史分位定位 = **盘**
- 本 SKILL.md = 每个指标的语义、历史阈值、组合 pattern、失效情形 = **解读**
- 决策 = Claude 读完盘面 + 查阅本手册 + 结合用户当前组合/风险偏好/宏观 context 综合给出

**v1 的错**（前身名 `coinman`）：曾经用 1 个 0-100 总分 + 6 类 action 试图替代上述综合判断。本质上把多维结构压成 1 维必然丢失信息，且阈值是拍脑袋的。**v2 更名「观复」**：不再产出评分 / action，仅给数据 + 解读手册。这正是"观复"两字的本意 —— 观象者不是工具，是主人。
...
/Users/x/news/bin/btc-guanfu --domain valuation
/Users/x/news/bin/btc-guanfu --domain network
/Users/x/news/bin/btc-guanfu --domain positioning
/Users/x/news/bin/btc-guanfu --domain macro
/Users/x/news/bin/btc-guanfu --domain flow

# AHR999 拟合半衰期（默认 1460 = 4 年）
/Users/x/news/bin/btc-guanfu --halflife 730   # 2 年，对快牛快熊敏感
```

冷启动 ~5-8s（拉 Binance + mempool + SoSoValue + F&G），缓存命中 <1s。
