...
## Track A — 读盘修复（剩 5 项 active；A2 ✅ Done eb69e98）
...
| **P0** | A1 | Gold/HS300 价格显示 $0.00 | `printEquityPanel` 行 427/429 硬编码 `SPYPrice+QQQPrice`；Gold 写入 `GoldPrice`，HS300 完全不写 price | 用 `panel.Asset`（A0 加）switch 选字段：qqq→QQQPrice、spy→SPYPrice、gold→GoldPrice、hs300→HS300Price；fallback 走 `BTCPrice`。所有 BuildPanel 路径必须填 `panel.Asset` | A0 | `cmd/guanfu/main.go` + 所有 `asset_*.go` BuildPanel |
...
| **P1** | A4 | HS300 读盘不显示 PMI/M2/LPR | `BuildHS300Dashboard` 只写 `cny_usd`；中国宏观数据 PMI/M2/LPR/Northbound 已存在 PriceStore (`hs300_pmi.json` 等) | **字段方案（不引 PriceStore 句柄进 dashboard）**：(1) `asset_hs300.go.BuildPanel` 用 `a.store.Latest("hs300_pmi"/"hs300_m2"/"hs300_lpr"/"hs300_northbound")` 预解析；(2) 在 `HS300DashboardInput` 加 `PMI/M2/LPR/Northbound float64` 字段；(3) `BuildHS300Dashboard` 保持纯函数，按字段写 `panel.Macro` | — | `asset_hs300.go`, `hs300_dashboard.go` |
| **P2** | A5 | QQQ/SPY 读盘缺 VIX context — **需先实测验证是否真有此 gap** | `equity_panel.go:103-108` 已写 `panel.Macro["vix_level"]` 当 `in.VIX > 0`；`asset_qqq.go:72/85` 已传 `as.CrossAssetPrices["vixy"]`；`vixy.json` 存在 → 理论上已 work | **第一步**：跑 `guanfu read qqq` / `read spy` 实测 panel.Macro 是否含 `vix_level`。若是，**删除 A5 整行**。若否，定位是 `vixy.json` 数据陈旧 / FetchSnapshot 没读 / dashboard 路径分支问题，再决定修法 | — | `asset_qqq.go`, `asset_spy.go`（仅在验证后有 gap 时） |
| **P3** | A6 | 任意 stock 读盘 | `readStock` 只有 forecast，无 panel | 复用 `BuildEquityPanel` 做技术面板（`USStockExtractors` 跑出 indicators） | **D2**（USStockExtractors）+ D3（readStock 改造）—— 已合入 D3，不独立 wave | `main.go` |
...
| 文件 | 改动 | 波次 |
|---|---|---|
| `cmd/guanfu/main.go` | printEquityPanel price 显示 (A1, 用 panel.Asset switch), readStock 升级 (D3), import-stock CLI (D4)；A2 已在 `eb69e98` 落地 | 0, 1 |
...
| `pkg/engine/asset_hs300.go` | BuildPanel 预解析 hs300_pmi/m2/lpr/northbound + 填 panel.Asset="hs300" (A0/A1/A4) | 0 |
| `pkg/engine/hs300_dashboard.go` | HS300DashboardInput 加 PMI/M2/LPR/Northbound 字段（字段方案，不引 PriceStore 句柄）(A4) | 0 |
| `pkg/engine/asset_qqq.go` / `asset_spy.go` | BuildPanel 填 panel.Asset (A0/A1)；A5 仅在实测验证有 gap 时才动 | 0 |
