...
| `stooq_putcall` | forecast | 5d | QQQ/SPY 期权情绪特征:ratio / 30d change / 252d percentile |
...
| `stooq_putcall` | Stooq `^PC` | CBOE total put/call ratio;QQQ/SPY forecast feature + panel positioning | 需 `STOOQ_APIKEY` |
...
| Stooq 不可用 | QQQ/SPY put/call 特征缺失,forecast bundle 自动降级 |
