...
> 30+ 数据集,覆盖 5 核心资产 + 任意美股 + 宏观 + 情绪。全部 JSON 日频存档在 `~/.guanfu/prices/`。统一 `guanfu refresh` 刷新,首次全量 / 后续增量。

---

## 一、统一刷新框架 (`guanfu refresh`)

23 个 Source 实现 `client.Source` 接口 (`Key / DisplayName / Refresh`),按 dependency order 串行执行。
...
- `last_date` 缺失 / > 24h → fetch `lastDate+1 → today`(FRED / Yahoo)或全量拉回 + Append dedup(AkShare)
- CAPE 月频 → 28 天 TTL
...
| `hs300` | 沪深 300 | AkShare 原生 | 日 | 2002-01-04 | 免费 |
...
### 中国宏观(6,AkShare bridge)

| Key | Series | 频率 |
|---|---|---|
| `hs300_pmi` | 官方制造业 PMI | 月 |
| `hs300_m2` | M2 同比 | 月 |
| `hs300_cpi` | CPI 同比 | 月 |
| `hs300_lpr` | LPR 1Y | 日 |
| `hs300_volume` | CSI300 日成交量 | 日 |
| `hs300_northbound` | 北向资金净买入 | 日 |

AkShare bridge: `scripts/akshare_bridge.py`(stdin JSON 协议,Python 3.8+ with akshare package)。
...
| `GUANFU_HISTORY_DB` | 指定 history.db 路径 | MCP Server 使用 |
| `GUANFU_SKILL_PATH` | MCP Resource SKILL.md 路径 | MCP 客户端自取上下文用 |
| `CACHE_DIR` | 缓存目录 | 默认系统用户缓存目录 |
| `GUANFU_BTC_KLINE_CACHE` | BTC 全历史日线缓存路径 | 默认 `$CACHE_DIR/btc_daily_history.json` |
| `FUTU_GATEWAY` | Futu OpenD 地址 | 默认 `127.0.0.1:11111` |
| `FUTU_ENABLED=0` | 禁用 Futu | 走 Yahoo 降级 |
| `FUTU_BRIDGE` | 自定义 futu_bridge.py 路径 | 覆盖自动探测 |
...
| AkShare 不可用 | Yahoo `000300.SS` 替代 CSI300(2021+,历史较短) |
...
│   ├── hs300_pmi.json / hs300_m2.json / hs300_lpr.json / ...
│   ├── spx_cape.json
│   ├── gold_cot.json
│   ├── vixy.json / tlt.json / uup.json / hs300_cny.json
...
7. 跑 `TestBacktestBundles` 确认回归预算内(见 `docs/guanfu-v3-todo.md` 回归预算表)
...
| **v3** | **2026-05-09** | **对齐 guanfu-v3-todo.md**:30+ 数据集,新 BTC ad-hoc 源独立 section,`source_health` 强化,数据目录结构明示 Track K/L 未来扩展点 |
