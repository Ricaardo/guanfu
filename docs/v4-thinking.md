...
第一次全量 23 个 source 预估 5-30 分钟,方差取决于 FRED/Yahoo 限流。
...
### U4. HS300 hard-block 对用户是增值还是折损?

当前:所有 HS300 horizon 拒绝输出数值。对中国用户可能是"我就是想看 HS300 怎么样啊 你咋啥都不给",对美国用户没影响。
→ **行动**:问一下真实中国用户,看 hard-block 输出用他们能不能接受的方式(把原始指标讲得更详细?给定性描述?)

### U5. Portfolio 真的有人配置吗?
...
- `guanfu refresh --dry-run` → 列出 30 个 source(含 v3 新增 9 个)
