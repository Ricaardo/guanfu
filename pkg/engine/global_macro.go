...
func EnrichGlobalInvestorMacro(panel *model.IndicatorPanel, ps *store.PriceStore) {
	...
	addUSDCNYMacro(panel, ps)
	addCentralBankMacro(panel, ps)
}

func addUSDCNYMacro(panel *model.IndicatorPanel, ps *store.PriceStore) {
...
func addCentralBankMacro(panel *model.IndicatorPanel, ps *store.PriceStore) {
	us, usOK := addLatestRate(panel, ps, "global_rate_us_fed_pct", "fred_fed_funds", "Fed effective rate", "fred:DFF")
	if !usOK {
		us, usOK = addLatestRate(panel, ps, "global_rate_us_fed_pct", "fred_dgs3mo", "US 3M T-bill proxy", "fred:DGS3MO")
	}
	eu, euOK := addLatestRate(panel, ps, "global_rate_eu_ecb_pct", "fred_ecb_deposit_rate", "ECB deposit rate", "fred:ECBDFR")
	jp, jpOK := addLatestRate(panel, ps, "global_rate_jp_boj_pct", "fred_boj_call_rate", "BOJ overnight call rate", "fred:IRSTCI01JPM156N")
	cn, cnOK := addLatestRate(panel, ps, "global_rate_cn_pboc_pct", "fred_pboc_interbank_rate", "PBoC interbank rate", "fred:IRSTCI01CNM156N")
...
func addLatestRate(panel *model.IndicatorPanel, ps *store.PriceStore, indicatorKey, storeKey, labelPrefix, fallbackSource string) (float64, bool) {
